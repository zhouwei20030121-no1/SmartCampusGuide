  ...
  <div class="page-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>校车路线管理</span>
          <el-button type="primary" @click="showDialog = true; editingLine = null">新增线路</el-button>
        </div>
      </template>
      <el-table :data="lines" border stripe v-loading="loading">
        ...
        <el-table-column label="操作" width="100">
          <template #default="{ row }">
            <el-button size="small" type="primary" link @click="editLine(row)">编辑</el-button>
    ...
    <el-dialog v-model="showDialog" :title="editingLine ? '编辑线路' : '新增线路'" width="500px">
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="首班时间"><el-time-picker v-model="form.startTime" format="HH:mm" /></el-form-item>
        <el-form-item label="末班时间"><el-time-picker v-model="form.endTime" format="HH:mm" /></el-form-item>
        ...
        <el-form-item label="支付方式"><el-input v-model="form.fareInfo" placeholder="如：一卡通/钉钉扫码" /></el-form-item>
...
const handleSave = async () => {
  const payload = {
    ...
    startTime: form.startTime
      ? (form.startTime as Date).toTimeString().slice(0, 8) : null,
    endTime: form.endTime
      ? (form.endTime as Date).toTimeString().slice(0, 8) : null,
  }
  try {
...
const delLine = async (id: number) => {
  ...
  try {
    await request.delete(`/bus/line/${id}`)
