...
class _ChatPageState extends State<ChatPage> {
  ...
  _RouteTarget? _normalizeRouteTarget(String destination) {
    if (destination.isEmpty) return null;
    final mappedTarget = _mappedRouteTarget(destination);
  ...
  _RouteTarget? _mappedRouteTarget(String destination) {
    final normalized = _normalizeRouteAlias(destination);
    if (normalized.isEmpty) return null;
    for (final mapping in _routeAliasMappings) {
      if (mapping.normalizedAliases.any(
        (alias) => alias == normalized || normalized.contains(alias),
      )) {
        return mapping.toRouteTarget();
      }
    }
    return null;
  }
  ...
  _RouteTarget _teachingBuildingTarget(String number) {
    ...
    if (normalizedNumber == '13' || normalizedNumber == '十三') {
      return const _RouteTarget(
        destination: '物理科学与技术学院',
        aliases: ['物理科学与技术学院', '物理学院', '立惠楼（第13教学楼）', '立惠楼', '第13教学楼', '13教'],
      );
    }
    return _RouteTarget(
  ...
  void _showVoiceInputSheet() {
    final voiceSamples = ['最近的食堂怎么走？', '电子信息工程学院在哪里？', '帮我规划一条参观校园的路线'];
...
class _RouteAliasMapping {
  final String destination;
  final List<String> aliases;

  const _RouteAliasMapping({required this.destination, required this.aliases});
  ...
  _RouteTarget toRouteTarget() {
    return _RouteTarget(destination: destination, aliases: aliases);
  }
}

const _routeAliasMappings = [
  _RouteAliasMapping(
    destination: '物理科学与技术学院',
    aliases: [
      '物理科学与技术学院',
