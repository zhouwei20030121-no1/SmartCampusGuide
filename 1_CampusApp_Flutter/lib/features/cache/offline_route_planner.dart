...
class OfflineRoutePlanner {
  ...
  static bool _hasCoords(SpotModel s) => s.latitude != 0 && s.longitude != 0;
  ...
  static List<SpotModel> planAdvancedRoute({
    ...
    if (strategy.toUpperCase() == 'PERSONALIZED' && waypoints.isEmpty) {
      ...
      if (start != null && end != null) {
        for (final s in _findTopPersonaSpots(
          start,
          end,
          valid,
          userIdentity,
  ...
  static List<SpotModel> _findTopPersonaSpots(
    ...
    final candidates =
        allSpots
            .where((s) => _hasCoords(s) && s.id != start.id && s.id != end.id)
            .where((s) {
          ...
          ..sort(
            (a, b) => _personaScore(
              b,
              identity,
            ).compareTo(_personaScore(a, identity)),
          );
