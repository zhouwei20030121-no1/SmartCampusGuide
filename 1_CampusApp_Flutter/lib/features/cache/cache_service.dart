...
class CacheService {
  ...
  static Future<bool> preloadAll({CacheProgressCallback? onProgress}) async {
    ...
    final spotsOk = await preloadSpots(onProgress: (p) {
      onProgress?.call('正在下载景点列表…', 0.05 + p * 0.35);
    });
    if (!spotsOk) return false;

    onProgress?.call('正在下载讲解内容…', 0.42);
    final guidesOk = await preloadGuides(onProgress: (p) {
    ...
    final routesOk = await preloadRoutes(onProgress: (p) {
  ...
  static Future<bool> preloadSpots({void Function(double progress)? onProgress}) async {
    lastError = null;
    try {
  ...
  static Future<bool> preloadGuides({void Function(double progress)? onProgress}) async {
    lastError = null;
    try {
        ...
        try {
          ...
          if (res.data['code'] == 200 && res.data['data'] != null) {
            ...
            guides.add({
              ...
              'script_content':
                  script.isNotEmpty ? script : _fallbackGuideText(spots[i]),
  ...
  static Future<bool> preloadRoutes({void Function(double progress)? onProgress}) async {
    lastError = null;
    try {
      final res = await NetworkClient.get('/route/plan/list', queryParameters: {
        'page': 1,
        'size': 100,
      });
  ...
  static Future<Map<String, dynamic>?> getCachedGuide(int spotId,
      {String language = 'zh'}) async {
    return LocalStorage.getGuideContent(spotId, language: language);
  }

  static Future<String?> getCachedGuideBySpotName(String spotName,
      {String language = 'zh'}) async {
    final spots = await getCachedSpots();
  ...
  static String _friendlyNetworkError(Object e) {
    ...
    if (text.contains('connection') ||
      ...
      return '网络连接失败。真机请用 --dart-define=API_BASE_URL=http://电脑局域网IP:8080 构建，'
          '或确保后端在 8080 端口运行。已尝试：${NetworkClient.apiBaseUrls.join('、')}';
    }
    return text;
  }
}
