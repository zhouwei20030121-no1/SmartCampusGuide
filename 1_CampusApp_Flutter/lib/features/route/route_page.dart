...
class _RoutePageState extends State<RoutePage> {
  ...
  late final MapUserLocationController _userLocation =
      MapUserLocationController.shared;
  ...
  Set<Marker> get _allMarkers => {..._markers.values, _userLocation.userMarker};
  ...
  Future<void> _loadMapData() async {
    ...
    initialStart ??= _findSpotByAliases(
      spots,
      _routeAliases(widget.initialStartName, widget.initialStartAliases),
    );
    initialStart ??= _defaultStartSpot(spots);
    ...
    final destinationName =
        ...
        : widget.initialEndName?.trim();
    final destinationAliases = _routeAliases(
      destinationName,
      widget.initialDestinationAliases,
    );
    initialEnd ??= _findSpotByAliases(spots, destinationAliases);

    if (initialEnd == null && destinationAliases.isNotEmpty) {
      final syntheticSpot = _syntheticSpotFor(destinationAliases);
      if (syntheticSpot != null) {
        spots = [...spots, syntheticSpot];
        initialEnd = syntheticSpot;
      }
    }
  ...
  LatLngBounds _spotMarkerBounds(List<SpotModel> spots) {
    if (spots.isEmpty) {
      return LatLngBounds(
        southwest: LatLng(
          _swuCenter.latitude - 0.012,
          _swuCenter.longitude - 0.014,
        ),
        northeast: LatLng(
          _swuCenter.latitude + 0.008,
          _swuCenter.longitude + 0.014,
        ),
      );
    }
  ...
  CameraPosition _cameraPositionForSpots(List<SpotModel> spots) {
    ...
    final zoom = (_estimateZoomForSpan(math.max(latSpan, lngSpan)) + 0.4).clamp(
      14.0,
      16.8,
    );
  ...
  List<String> _routeAliases(String? primary, List<String>? aliases) {
    final values = <String>[
      if (primary != null) primary,
      ...?aliases,
      if (primary != null) ..._knownAliasesFor(primary),
    ];
    return values
        .map((item) => item.trim())
  ...
  SpotModel? _defaultStartSpot(List<SpotModel> spots) {
    final userPos = _userLocation.userPos;
    final hasMovedUserLocation =
        (userPos.latitude - CampusBounds.defaultCenter.latitude).abs() >
            0.000001 ||
        (userPos.longitude - CampusBounds.defaultCenter.longitude).abs() >
            0.000001;
    if (hasMovedUserLocation) {
      final nearest = _nearestSpotTo(spots, userPos);
      if (nearest != null) return nearest;
    }
    return _findSpotByAliases(spots, const ['学行门（2号门）', '学行门', '二号门', '2号门']) ??
        _nearestSpotTo(spots, CampusBounds.defaultCenter);
  }

  SpotModel? _nearestSpotTo(List<SpotModel> spots, LatLng point) {
    SpotModel? nearest;
  ...
  List<String> _knownAliasesFor(String placeName) {
    final normalized = _normalizeSpotName(placeName);
  ...
  SpotModel? _syntheticSpotFor(List<String> aliases) {
    final normalizedText = aliases.map(_normalizeSpotName).join(' ');
    ...
    for (final spot in spots) {
      batch[spot.id.toString()] = Marker(
        ...
        icon: BitmapDescriptor.defaultMarkerWithHue(
          _markerHueForCategory(spot.category),
        ),
  ...
  Future<void> _persistRouteHistory(
    int generation,
    _RoutePlanSnapshot snapshot,
  ) async {
    if (!mounted || generation != _routePlanGeneration) return;

    final path =
        _cachedPaths[snapshot.strategy] ?? _cachedPaths['DISTANCE'] ?? [];
  ...
  void _drawCurrentRoute({bool autoZoom = false}) {
    ...
    setState(() {
      ...
      if (_currentStrategy == 'DISTANCE')
        ...
        routeColor = Colors.purpleAccent;
    ...
    if (autoZoom && activePath.isNotEmpty) {
      Future.delayed(const Duration(milliseconds: 300), () {
        ...
        _mapController?.moveCamera(
          CameraUpdate.newLatLngBounds(_calculateBounds(activePath), 100.0),
          animated: true,
        );
      });
    }
  ...
  LatLngBounds _calculateBounds(List<LatLng> points) {
    if (points.isEmpty)
      return LatLngBounds(southwest: _swuCenter, northeast: _swuCenter);
  ...
  Widget _buildIdentitySelector() {
    ...
    return Row(
      children: identities.entries.map((entry) {
        ...
        return Expanded(
          child: Padding(
            padding: EdgeInsets.only(right: entry.key != 'ALUMNI' ? 6 : 0),
            child: InkWell(
              onTap: () {
